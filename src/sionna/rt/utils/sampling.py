...
def compute_signal_strength_at_positions(
    ...
    :param positions: Positions at which to compute signal strength
        [num_samples, 3]
    ...
    :return: Tuple of (positions_np, path_gain, rss, sinr)
        ...
        - rss: numpy array of received signal strength in dBm
          [num_tx, num_samples]
    ...
    for i in range(num_samples):
        ...
        rx = Receiver(name=rx_name, position=pos,
    ...
    try:
        ...
        tx_powers = np.array([
            tx.power[0] for tx in scene.transmitters.values()
        ])
        ...
        # Convert RSS to dBm (suppress warnings for zero/negative values)
        ...
        # [1, num_samples]
        total_power = np.sum(rss_watts, axis=0, keepdims=True)

        # Interference for each transmitter (total - own signal)
        interference = total_power - rss_watts  # [num_tx, num_samples]

        # Thermal noise power - extract scalar from Dr.Jit type
        noise_power_val = scene.thermal_noise_power
        if hasattr(noise_power_val, 'numpy'):
            noise_power_arr = noise_power_val.numpy()
            if noise_power_arr.ndim > 0:
                noise_power = float(noise_power_arr.item())
            else:
                noise_power = float(noise_power_arr)
        else:
            if hasattr(noise_power_val, '__getitem__'):
                noise_power = float(noise_power_val[0])
            else:
                noise_power = float(noise_power_val)
        ...
        def _extract_coord(coord, num_samples):
            if hasattr(coord, 'numpy'):
                return coord.numpy()
            return np.array([float(coord[i]) for i in range(num_samples)])

        positions_np = np.column_stack([
            _extract_coord(positions.x, num_samples),
            _extract_coord(positions.y, num_samples),
            _extract_coord(positions.z, num_samples)
        ])
...
def sample_signal_data(
    ...
    :param seed: Random seed for reproducibility
    :param bounds: Optional custom sampling bounds as
        (min_x, max_x, min_y, max_y). If None, uses scene bounding box.
    :param max_depth: Maximum number of reflections/interactions for
        ray tracing
    :param los: Enable line-of-sight paths
    :param specular_reflection: Enable specular reflection
    :param diffuse_reflection: Enable diffuse reflection
    ...
    :param diffraction: Enable diffraction
    :param edge_diffraction: Enable edge diffraction
    :param synthetic_array: Use synthetic array modeling for
        efficiency
    :param samples_per_src: Number of rays per source for path
        tracing
    :param output_format: Output format: "dict" or "list"
        - "dict": Returns a dictionary with arrays for each
          metric
        - "list": Returns a list of dictionaries, one per sample
          point
    ...
    .. code-block:: python
        ...
        scene.add(rt.Transmitter(
            name="tx1",
            position=mi.Point3f(10, 20, 30)
        ))
        scene.add(rt.Transmitter(
            name="tx2",
            position=mi.Point3f(-10, 30, 25)
        ))
        ...
        data = sample_signal_data(
            scene, num_samples=500, z_height=1.5
        )
    ...
    (positions_np, path_gain,
     rss_dbm, sinr_db) = compute_signal_strength_at_positions(
        scene, positions,
        max_depth=max_depth,
        los=los,
        specular_reflection=specular_reflection,
